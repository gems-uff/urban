...
public static void rodarVertex(int i){
    ...
    if(ProvenanceMainCombined.linhaIdBus[posicaoLin][4].equals("0"))
    {        
    ...
    */
    ...
    if(ProvenanceMainCombined.linhaIdBus[posicaoLin][4].equals("0"))
    {
        ProvenanceMainCombined.linhaIdBus[posicaoLin][4] = "1";
    ...
    value4_vertex.setText("" + ProvenanceMainCombined.TabelaDePontos[i][0]);
    ...
    value5_vertex.setText("" + ProvenanceMainCombined.TabelaDePontos[i][1]);
    ...
    value6_vertex.setText("" + ProvenanceMainCombined.TabelaDePontos[i][2]);
    ...
    Element name7_vertex = new Element("name");
    name7_vertex.setText("Number of Vertices");
    attribute_vertex.addContent(name7_vertex);
    
    //======================================
    
    Element value7_vertex = new Element("value");
    value7_vertex.setText(ProvenanceMainCombined.TabelaDePontos[i][3] + "");
    attribute_vertex.addContent(value7_vertex);
...
public static void rodarEdge(int k,  BusPosition busPosition)
{
    ...
    id_edge.setText("edge_" + ProvenanceMainCombined.contador_de_edges);
    ...
    while(!ProvenanceMainCombined.linhaIdBus[pos][0].equals("") || !ProvenanceMainCombined.linhaIdBus[pos][1].equals(""))
    {
        //JOptionPane.showMessageDialog(null, "" + provenanceM.linhaIdBus[pos][0]);
        if(ProvenanceMainCombined.linhaIdBus[pos][0].equals("Linha") 
        && ProvenanceMainCombined.linhaIdBus[pos][1].equals("" + busPosition.getBusId()))
        {            
            VelocidadeAntiga = Double.parseDouble(ProvenanceMainCombined.linhaIdBus[pos][2]);
            ProvenanceMainCombined.linhaIdBus[pos][2] = "" + VelocidadeAtual;
            //
            idEdgeAnterior = Integer.parseInt(ProvenanceMainCombined.linhaIdBus[pos][3]);
            idEdgeAtual = k;
            ProvenanceMainCombined.linhaIdBus[pos][3] = "" + idEdgeAtual;
            break;
        }
    ...
    id_edge.setText("edgeDeltaV_" + ProvenanceMainCombined.contador_de_edges);
    edge.addContent(id_edge);
    ...
    ProvenanceMainCombined.contador_de_edges = ProvenanceMainCombined.contador_de_edges + 1;
      ...
      public static void mainGeneratingXML(String linhaIdBus[][]) {
          ...
          //agenda.addContent("thiago");
           ...
           params.put("bus_id", "5");
            ...
            busPositionDao.setStatement(stmt);
            String params;
            params = ProvenanceMainCombined.String_SQL;
            ...
            //Variáveis para controle das distâncias entre os pontos
            double menor_distancia_long = 9999;
            double menor_distancia_lat = 9999;
            double aux_menor_distancia_long = 0;
            double aux_menor_distancia_lat = 0;
            double aux_hipotenusa = 0;
            double hipotenusa = 0;
            int linha_menor_distancia = -1;
            
            //Formando a tabela de pontos combinados
            for (BusPosition busPosition : busPositions) 
            {                
                linha_menor_distancia = -1;//inicializa toda vez
                menor_distancia_long = 9999;//inicializa toda vez
                menor_distancia_lat = 9999;//inicializa toda vez
                
                if(nVertex < ProvenanceMainCombined.QUANTIDADE_DE_PONTOS)
                {
                    for(int linha = 0; linha < ProvenanceMainCombined.LIN_TabelaDePontos; linha++)
                    {
                        if(ProvenanceMainCombined.TabelaDePontos[linha][3] == 0 && linha_menor_distancia == -1)
                        {
                            ProvenanceMainCombined.TabelaDePontos[linha][0] = busPosition.getLongitude();
                            ProvenanceMainCombined.TabelaDePontos[linha][1] = busPosition.getLatitude();
                            ProvenanceMainCombined.TabelaDePontos[linha][2] = busPosition.getSpeed();
                            ProvenanceMainCombined.TabelaDePontos[linha][3] = 1;
                            ProvenanceMainCombined.TabelaDePontos[linha][4] = busPosition.getLineId();
                            break;
                        }
                        else
                        {
                            if(
                               ProvenanceMainCombined.TabelaDePontos[linha][0] - ProvenanceMainCombined.raio_aceitavel >= busPosition.getLongitude() &&
                               ProvenanceMainCombined.TabelaDePontos[linha][0] + ProvenanceMainCombined.raio_aceitavel <= busPosition.getLongitude() &&
                               ProvenanceMainCombined.TabelaDePontos[linha][1] - ProvenanceMainCombined.raio_aceitavel >= busPosition.getLatitude() &&
                               ProvenanceMainCombined.TabelaDePontos[linha][1] + ProvenanceMainCombined.raio_aceitavel <= busPosition.getLatitude()
                              )
                            {
                                if(linha_menor_distancia == -1)
                                {                                
                                    menor_distancia_long = ProvenanceMainCombined.TabelaDePontos[linha][0] - busPosition.getLongitude();
                                    if(menor_distancia_long < 0){
                                        menor_distancia_long = (-1) * menor_distancia_long;
                                    }
                                
                                    menor_distancia_lat = ProvenanceMainCombined.TabelaDePontos[linha][0] + busPosition.getLongitude();
                                    if(menor_distancia_lat < 0){
                                        menor_distancia_lat = (-1) * menor_distancia_lat;
                                    }
                                
                                    linha_menor_distancia = linha;
                                }
                                else
                                {                                    
                                    aux_menor_distancia_long = ProvenanceMainCombined.TabelaDePontos[linha][0] - busPosition.getLongitude();
                                    if (aux_menor_distancia_long < 0) {
                                        aux_menor_distancia_long = (-1) * aux_menor_distancia_long;
                                    }

                                    aux_menor_distancia_lat = ProvenanceMainCombined.TabelaDePontos[linha][0] + busPosition.getLongitude();
                                    if (aux_menor_distancia_lat < 0) {
                                        aux_menor_distancia_lat = (-1) * aux_menor_distancia_lat;
                                    }

                                    aux_hipotenusa = Math.sqrt((aux_menor_distancia_long * aux_menor_distancia_long) + (aux_menor_distancia_lat * aux_menor_distancia_lat));
                                    hipotenusa = Math.sqrt((menor_distancia_long * menor_distancia_long) + (menor_distancia_lat * menor_distancia_lat));

                                    if (aux_hipotenusa < hipotenusa) {
                                        menor_distancia_long = ProvenanceMainCombined.TabelaDePontos[linha][0] - busPosition.getLongitude();
                                        if (menor_distancia_long < 0) {
                                            menor_distancia_long = (-1) * menor_distancia_long;
                                        }
                        ...
                        if(ProvenanceMainCombined.TabelaDePontos[linha][3] == 0 && linha_menor_distancia != -1)
                        {
                            ProvenanceMainCombined.TabelaDePontos[linha_menor_distancia][2] = ProvenanceMainCombined.TabelaDePontos[linha_menor_distancia][2] + busPosition.getSpeed();
                            ProvenanceMainCombined.TabelaDePontos[linha_menor_distancia][3] = ProvenanceMainCombined.TabelaDePontos[linha_menor_distancia][3] + 1;
                            break;
                        }
                    }                        
                }else{
            ...
            //Fazendo a Média das Médias
            for(int linha = 0; linha < ProvenanceMainCombined.LIN_TabelaDePontos; linha++)
            {                
                ProvenanceMainCombined.TabelaDePontos[linha][2] = ProvenanceMainCombined.TabelaDePontos[linha][2] / ProvenanceMainCombined.TabelaDePontos[linha][3];
            }
            ...
            for (int linha = 0; linha < ProvenanceMainCombined.LIN_TabelaDePontos; linha++) 
            {
              ...
              while (!ProvenanceMainCombined.linhaIdBus[pos][0].equals("") || !ProvenanceMainCombined.linhaIdBus[pos][1].equals("")) {
                  ...
                  if (ProvenanceMainCombined.linhaIdBus[pos][0].equals("Linha")
                          && ProvenanceMainCombined.linhaIdBus[pos][1].equals("" + busPosition.getBusId())
                          && ProvenanceMainCombined.linhaIdBus[pos][5].equals("0")) {
                      ProvenanceMainCombined.linhaIdBus[pos][2] = "" + VelocidadeAtual;
                      ProvenanceMainCombined.linhaIdBus[pos][3] = "" + nVertex;

                      ProvenanceMainCombined.linhaIdBus[pos][5] = "1";//avisa que é a primeira vez que essa linha aparece pedindo edge, 
